        ...
        if args.triage:
            ...
            if persona == 'po':
                ...
                triage_question = (
                    ...
                    f"Your role is to CATEGORIZE and ROUTE paths to owners — NOT to diagnose or prescribe fixes.\n\n"
                    f"Here is the triage data (already computed — do NOT call triage_timing_run):\n"
                    f"{triage_json}\n\n"
                    f"IMPORTANT: {po_int_count} PO_INT paths and {pteco_count} PTECO paths are already\n"
                    f"auto-bucketed by Python. Do NOT include them in your bucket list.\n"
                    f"Python will automatically merge them into the export.\n\n"
                    f"Your job: create buckets ONLY for the {remaining} remaining C2C/EXT paths.\n\n"
                    f"CLASSIFICATION RULES (use these labels exactly):\n"
                    f"  - CLASSIF_CONSTRAINTS: Paths with constraint issues (IOC/CON rules).\n"
                    f"    Signs: unrealistic slack, clock definition issues, missing exceptions,\n"
                    f"    IO delay problems, cross-clock without proper setup, clock_percentage > 100%.\n"
                    f"  - CLASSIF_FCT: Feedthrough / section-level issues (FTC/HRP/MOP/SKW rules).\n"
                    f"    Signs: feedthrough paths, high logic depth, long nets, clock skew.\n"
                    f"  - Partition_Internals: Paths where BOTH startpoint and endpoint belong to\n"
                    f"    the SAME partition (int_ext='INT', same driver/receiver partition).\n"
                    f"    These are the PO's responsibility. Do NOT diagnose — just label and route.\n"
                    f"  - CLASSIF_PTECO: Paths with < 2% clock window. Already auto-bucketed.\n\n"
                    f"CRITICAL RULES:\n"
                    f"  - NEVER mix INT and EXT paths in the same bucket.\n"
                    f"    Check the int_ext column in the data. If a group has both, split it.\n"
                    f"  - For Partition_Internals buckets: group by clock domain + partition.\n"
                    f"    Description = partition name + clock domain + path count. Keep it short.\n"
                    f"  - For CLASSIF_CONSTRAINTS / CLASSIF_FCT: identify the IRIS rule ID\n"
                    f"    and briefly state what's wrong (1 sentence). No fix prescriptions.\n"
                    f"  - Do NOT include PathType in filters — it is added automatically.\n\n"
                    f"Workflow:\n"
                    f"1. Use remaining_c2c_ext groups, prefixes, and worst paths to create buckets.\n"
                    f"   Do NOT call query_timing_db — the data is already summarized.\n"
                    f"2. For each group, check int_ext: if INT → Partition_Internals. If EXT → classify.\n"
                    f"3. For each bucket: filters MUST include StartPin and/or EndPin regex.\n"
                    f"4. Call export_bucket_file({export_params}, buckets=<your C2C/EXT buckets only>).\n"
                    f"   Python will prepend the {len(po_int_buckets) + len(pteco_buckets)} auto-buckets.\n"
                    f"5. Print triage summary table: bucket #, classification, path count, worst slack."
                )
